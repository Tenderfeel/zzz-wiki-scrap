...
/**
 * メイン処理パイプライン
 * Scraping.md解析からcharacters.ts出力までの完全フローを実装
 * 要件: 1.1, 5.5, 7.4
 */
export class MainPipeline {
  ...
  /**
   * 全体処理パイプラインを実行
   * Scraping.md解析からcharacters.ts出力までの完全フロー
   * 各処理段階の順次実行
   * 処理結果の統計情報表示
   * 要件: 1.1, 5.5, 7.4
   */
  async execute(options: PipelineOptions = {}): Promise<PipelineResult> {
    ...
    try {
      ...
      const characterEntries = await this.parser.parseScrapingFile(
      ...
      const processingResult = await this.batchProcessor.processAllCharacters(
        characterEntries,
        {
      ...
      if (!validationResult.isValid) {
        throw new AllCharactersError(
          ...
          null,
          `Character配列の検証に失敗しました: 重複ID ${validationResult.duplicateIds.length}件, 無効キャラクター ${validationResult.invalidCharacters.length}件`
        );
      }
      ...
      // エラーを再スローする前に、部分的な結果があれば保存を試みる
      if (
        error instanceof AllCharactersError &&
        error.stage !== ProcessingStage.PARSING
      ) {
...
`;
  ...
  async retryFailedCharacters(
    ...
    if (previousResult.failed.length === 0) {
    ...
    console.log(`\n🔄 === 失敗キャラクターの再処理 ===`);
    console.log(`対象キャラクター数: ${previousResult.failed.length}`);
    console.log(
      `失敗キャラクター: ${previousResult.failed
        .map((f) => f.entry.id)
        .join(", ")}`
    );
    console.log(`================================\n`);
    ...
    try {
      ...
      // バッチ処理で再実行
      const retryResult = await this.batchProcessor.processAllCharacters(
        failedEntries,
        {
          batchSize: opts.batchSize,
          delayMs: opts.delayMs,
      ...
      // 成功したキャラクターがあれば、既存の成功結果と統合
      const allSuccessful = [
        ...previousResult.successful,
      ...
      if (allSuccessful.length > 0) {
        ...
        return {
          ...
          processingResult: {
            ...
            statistics: {
              total: allSuccessful.length + retryResult.failed.length,
      ...
      return {
        ...
        outputFilePath: opts.outputFilePath,
        executionTime,
        success: retryResult.successful.length > 0,
      };
    } catch (error) {
      const endTime = Date.now();
      const executionTime = endTime - startTime;
