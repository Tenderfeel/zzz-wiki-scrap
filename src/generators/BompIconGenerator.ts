...
import {
  ...
  BompIconProcessingResult,
} from "../types/processing";
import { logger, LogMessages } from "../utils/Logger";
import { BompIconBatchError } from "../errors";
import { promises as fs } from "fs";
import path from "path";

/**
 * ボンプアイコンダウンロードの統合処理を担当するクラス
 * 全32のボンプアイコンを効率的にダウンロードし、統計情報を提供する
 * 要件: 1.4, 5.5
 */
export class BompIconGenerator {
  private readonly bompListParser: BompListParser;
  private readonly bompIconProcessor: BompIconProcessor;
  private readonly config: BompIconConfig;

  constructor(
    bompListParser: BompListParser,
    bompIconProcessor: BompIconProcessor,
    config: BompIconConfig
  ) {
    this.bompListParser = bompListParser;
    this.bompIconProcessor = bompIconProcessor;
    this.config = config;
  }

  /**
   * 全ボンプアイコンのダウンロード処理を実行
   * 要件: 1.4, 5.5
   */
  async generateBompIcons(): Promise<BompIconProcessingResult> {
    ...
    try {
      ...
      // Scraping.mdからボンプエントリを取得
      const bompEntries = await this.bompListParser.parseScrapingFile(
        "Scraping.md"
      );

      logger.info(`${bompEntries.length}個のボンプエントリを取得しました`, {
        bompIds: bompEntries.map((entry) => entry.id),
      });

      // バッチ処理でアイコンをダウンロード
      const results = await this.processBompsBatch(bompEntries);

      // 結果を集計
      const successful: BompIconInfo[] = [];
      const failed: Array<{
        bompId: string;
        error: string;
        iconUrl?: string;
      ...
      const processingResult: BompIconProcessingResult = {
        successful,
        failed,
        statistics: {
          total: bompEntries.length,
      ...
      // バッチエラーの場合は詳細情報を追加
      if (error instanceof BompIconBatchError) {
  ...
  async processBompsBatch(
    ...
    const results: BompIconDownloadResult[] = [];
    const batchSize = this.config.maxConcurrency;
    const totalBatches = Math.ceil(bompEntries.length / batchSize);
    const startTime = Date.now();
    ...
    logger.info(LogMessages.BOMP_ICON_BATCH_START, {
      batchSize,
      totalBomps: bompEntries.length,
      totalBatches,
    ...
    // バッチごとに処理
    for (let i = 0; i < bompEntries.length; i += batchSize) {
      const batch = bompEntries.slice(i, i + batchSize);
      const batchNumber = Math.floor(i / batchSize) + 1;

      logger.info(`バッチ ${batchNumber}/${totalBatches} 処理開始`, {
        batchSize: batch.length,
        bompIds: batch.map((entry) => entry.id),
      });

      // 並行処理でバッチ内のボンプを処理
      const batchPromises = batch.map(async (bompEntry, index) => {
        // バッチ内でも少し遅延を入れてリクエストを分散
        if (index > 0) {
          await this.sleep(100 * index); // 100ms * インデックス
        }
        return this.bompIconProcessor.processBompIcon(bompEntry);
      });
      ...
      try {
        ...
        logger.info(LogMessages.BOMP_ICON_BATCH_INDIVIDUAL_FALLBACK, {
          batchNumber,
          batchSize: batch.length,
        });

        const individualResults: BompIconDownloadResult[] = [];
        for (const bompEntry of batch) {
          try {
            const result = await this.bompIconProcessor.processBompIcon(
              bompEntry
            );
      ...
      // バッチ間の遅延（最後のバッチ以外）
      if (i + batchSize < bompEntries.length) {
  ...
  async validateOutputDirectory(): Promise<void> {
    try {
      ...
      logger.debug(LogMessages.BOMP_ICON_OUTPUT_DIRECTORY_VALIDATION, {
        outputDirectory: this.config.outputDirectory,
        status: "exists",
      });
    } catch {
      logger.info(LogMessages.BOMP_ICON_OUTPUT_DIRECTORY_CREATED, {
        outputDirectory: this.config.outputDirectory,
      });
    ...
    try {
      ...
      logger.debug(LogMessages.BOMP_ICON_OUTPUT_DIRECTORY_WRITE_TEST, {
        outputDirectory: this.config.outputDirectory,
        status: "success",
      });
    } catch (error) {
      const writeError = new BompIconBatchError(
        [],
        0,
      ...
      logger.error(LogMessages.BOMP_ICON_OUTPUT_DIRECTORY_WRITE_TEST, {
        outputDirectory: this.config.outputDirectory,
        status: "failed",
  ...
  logProcessingStatistics(result: BompIconProcessingResult): void {
    ...
    logger.info(LogMessages.BOMP_ICON_STATISTICS_SUMMARY, {
    ...
    logger.info(LogMessages.BOMP_ICON_PERFORMANCE_METRICS, {
  ...
  private logBatchProgress(
    batchNumber: number,
    totalBatches: number,
    batchResults: BompIconDownloadResult[],
    overallStats: {
    ...
    logger.info(LogMessages.BOMP_ICON_BATCH_PROGRESS, {
      progress: {
  ...
  private collectDetailedStatistics(
    results: BompIconDownloadResult[],
    processingTimeMs: number
  ): BompIconProcessingResult["statistics"] & {
    errorAnalysis: {
